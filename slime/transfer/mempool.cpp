...
MemoryPool::MemoryPool(size_t pool_size, size_t block_size, struct ibv_pd *pd)
    : pool_(nullptr), pool_size_(pool_size), block_size_(block_size), pd_(pd),
      mr_(nullptr), last_search_position_(0), allocated_blocks_(0) {
  // calculate total blocks
  total_blocks_ = pool_size_ / block_size_;
  assert(pool_size % block_size == 0);

  SLIME_LOG_INFO(
      "Memory pool size: ", pool_size_, " bytes, block size: ", block_size_,
      " bytes, total blocks: ", total_blocks_, ", it may take a while");
  if (posix_memalign(&pool_, 4096, pool_size_) != 0) {
    SLIME_ERROR("Failed to allocate memory pool");
    exit(EXIT_FAILURE);
  }

  SLIME_LOG_INFO("Memory pool allocated at ", pool_);
  ...
  if (!mr_) {
    SLIME_ERROR("Failed to register MR");
    exit(EXIT_FAILURE);
  }
...
int MemoryPool::allocate(size_t size, size_t n,
                         SimpleAllocationCallback callback) {
  ...
  for (size_t word_index = last_search_position_; word_index < bitmap_.size();
       ++word_index) {
    if (num_allocated == n) {
      break;
    }
    ...
    for (size_t bit_index = __builtin_ctzll(~word); bit_index < bit_per_word;
         ++bit_index) {
...
void MemoryPool::deallocate(void *ptr, size_t size) {
  ...
  if (offset % block_size_ != 0) {
    std::cerr << "Invalid pointer deallocation attempt: not aligned"
              << std::endl;
    return;
  }
  ...
  if (start_block >= total_blocks_) {
    SLIME_ERROR("Pointer out of range");
    return;
  }

  if (start_block + blocks_to_free > total_blocks_) {
    SLIME_ERROR("Deallocation size out of range");
    return;
  }
  ...
  for (size_t i = start_block; i < start_block + blocks_to_free; ++i) {
    ...
    if (bitmap_[idx] & (1ULL << bit)) {
      ...
      SLIME_ERROR("Double free detected at block index {}", i);
    }
  }
...
void MM::add_mempool(struct ibv_pd *pd) {
  mempools_.push_back(
      new MemoryPool((size_t)EXTEND_POOL_SIZE, (size_t)EXTEND_BLOCK_SIZE, pd));
}
...
bool MM::allocate(size_t size, size_t n, AllocationCallback callback) {
  ...
  for (int i = 0; i < mempool_cnt; ++i) {
    ...
    auto simple_callback = [callback, i](void *ptr, uint32_t lkey,
                                         uint32_t rkey) {
    ...
    SLIME_LOG_DEBUG("Mempool Count: " + std::to_string(mempool_cnt) +
                        ", Pool idx: " + std::to_string(i) + ", Total blocks: ",
                    total_blocks, ", allocated blocks: ", allocated_blocks,
                    ", block usage: ", 100 * allocated_blocks / total_blocks);
