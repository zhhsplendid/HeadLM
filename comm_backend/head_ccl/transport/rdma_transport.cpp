...
int64_t RDMAContext::cq_poll_handle()
{
    ...
    if (!connected_) {
        HEADLM_LOG_ERROR("Start CQ handle before connected, please construct first");
        return -1;
    }
        ...
        while (size_t nr_poll = ibv_poll_cq(cq_, POLL_COUNT, wc)) {
            ...
            for (size_t i = 0; i < nr_poll; ++i) {
                ...
                if (wc[i].status == IBV_WC_SUCCESS) {
                    ...
                    HEADLM_LOG_ERROR("WR failed with status: " << ibv_wc_status_str(wc[i].status) << std::endl);
...
int64_t RDMAContext::init_rdma_context(std::string dev_name, uint8_t ib_port, std::string link_type)
{
    ...
    if (!ib_ctx_) {
        HEADLM_LOG_WARN("Can't find or failed to open the specified device, try to open "
                       "the default device "
