...
#define STREAM_VAR_ARGS8(a, b, c, d, e, f, g, h)                               \
  << a << b << c << d << e << f << g << h

#define GET_MACRO(_1, _2, _3, _4, _5, _6, _7, _8, NAME, ...) NAME

#define STREAM_VAR_ARGS(...)                                                   \
  GET_MACRO(__VA_ARGS__, STREAM_VAR_ARGS8, STREAM_VAR_ARGS7, STREAM_VAR_ARGS6, \
            STREAM_VAR_ARGS5, STREAM_VAR_ARGS4, STREAM_VAR_ARGS3,              \
            STREAM_VAR_ARGS2, STREAM_VAR_ARGS1)                                \
  (__VA_ARGS__)
  ...
  {                                                                            \
    if (get_log_level() >= Level) {                                            \
      std::cerr << ": \033[1;91m"                                              \
                << "[" << MsgType << "]"                                       \
                << "\033[m " << __FILE__ << ": " << __FUNCTION__ << ": Line"   \
                << __LINE__ << ": " __VA_OPT__(STREAM_VAR_ARGS(__VA_ARGS__))   \
                << std::endl;                                                  \
    }                                                                          \
  }
