...
async def main():
    ...
    buffer_tensor = torch.zeros(buffer_shape, device="cuda", dtype=torch.float32)

    engine = TransferEngine("mlx5_bond_0", 1, "Ethernet")
    session_id = 0
    engine.init_link(session_id)
    future = engine.buffered_receive_tensor(session_id, test_tensor, buffer_tensor, recv_indices, "localhost", 3344, 4433)
    await future
    print(buffer_tensor)
    print(test_tensor)
